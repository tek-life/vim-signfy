...
let s:vcs_list       = get(g:, 'signify_vcs_list', [ 'git', 'perforce', 'hg', 'svn', 'darcs', 'bzr', 'fossil', 'cvs', 'rcs' ])
...
" Function: s:repo_get_diff_perforce {{{1
function! s:repo_get_diff_perforce(path) abort
  if executable('p4')
    let diff = system('cd '. s:escape(fnamemodify(a:path, ':h')) .' && p4 diff -du0 '. s:escape(a:path))
    return v:shell_error ? '' : diff
  endif
endfunction
...
highlight DiffDelete        cterm=bold ctermbg=none ctermfg=40
highlight DiffChange        cterm=bold ctermbg=none ctermfg=40
